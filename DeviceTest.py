...
class DeviceTestCase(unittest.TestCase):
    ...
    def test_write_line(self):
        self.assertRaises(PermissionError, write_line, open_device('/devices/dev0'), 'Tomsk')
        device: Device = open_device('/devices/dev2')
        write_line(device, 'Tomsk')
        self.assertEqual('Tomsk', read_line(device))
        self.assertRaises(IOError, write_line, open_device('/devices/dev4'), 'Tomsk')
